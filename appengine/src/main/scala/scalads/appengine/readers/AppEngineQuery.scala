...
class AppengineQuery[U](ds: AbstractDatastore, gQuery: GQuery, deserializer: (AbstractDatastore, ObjectReader) => U with EntityBacker[U]) { self =>

  protected val fetchOptions = FetchOptions.Builder.withDefaults()


  def limit(size: Int): this.type = new Query[U](ds, gQuery, deserializer) {
  ...
  def mapIterator[T](f: (AbstractDatastore, ObjectReader) => T): QueryIterator[T] = new QueryIterator(runQuery, ds, f)
