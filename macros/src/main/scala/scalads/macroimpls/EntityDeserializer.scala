...
object EntityDeserializer {
  ...
  def extendWithEntityBacker[U: c.WeakTypeTag, E: c.WeakTypeTag, DS <: AbstractDatastore {type Entity = E}: c.WeakTypeTag]
  (c: Context)(ds: c.Expr[DS], reader: c.Expr[ObjectReader]): c.Expr[U with EntityBacker[U, E]] = {
    ...
    val newDatastoreTree = ValDef(Modifiers(), dsName, TypeTree(weakTypeOf[DS]), ds.tree)
    ...
    val newTree = Block(List(
      ...
      ClassDef(Modifiers(Flag.FINAL), newTypeName("$anon"), List(), Template(List(appliedTpeTree, AppliedTypeTree(Ident(backerSym), List(appliedTpeTree, TypeTree(weakTypeOf[E])))),
        ValDef(Modifiers(Flag.PRIVATE), newTermName("self"), TypeTree(), EmptyTree) , List(
          ...
          ValDef(Modifiers(), newTermName("ds"), TypeTree(weakTypeOf[DS]), dsExpr.tree),
